...
contract("LunaToken", function(accounts) {
    var tokenInstance;

    it("Initializes the contract with the correct values", function() {
      return LunaToken.deployed().then(function(instance) {
        tokenInstance = instance;
        return tokenInstance.name();
      }).then(function(name) {
        assert.equal(name, "Luna Token", "has the correct name");
        return tokenInstance.symbol();
      }).then(function(symbol) {
        assert.equal(symbol, "LUNA", "has the correct symbol");
      });
    });
    ...
    it('handles delegated token transfers', function() {
      return LunaToken.deployed().then(function(instance) {
        ...
        assert.equal(receipt.logs.length, 1, 'triggers one event');
        assert.equal(receipt.logs[0].event, 'Transfer', 'should be the "Transfer" event');
        assert.equal(receipt.logs[0].args.from, fromAccount, 'Logs the account the tokens are authorized by');
        assert.equal(receipt.logs[0].args.to, toAccount, "Logs the account the tokens are transfered to");
